...
const ShowScheduleEidt: FC<parentType> = ({schedule, updateSchedule}) => {
    const getIcon = () => {
        if(schedule.index != -1)
            return <Icon name="calendar-minus" size={32} color='black' onPress={() => {updateSchedule(schedule)}}/>
        return <Icon name="calendar-plus" size={32} color='black' onPress={() => {updateSchedule(schedule)}}/>
    }
